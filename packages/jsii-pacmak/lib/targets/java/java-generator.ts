...
export class JavaGenerator extends Generator {
  ...
  private emitMultiplyInheritedOptionalProperties(ifc: spec.InterfaceType) {
    ...
    function allOptionalProps(this: JavaGenerator, fqn: string) {
      ...
      for (const prop of type.properties ?? []) {
        // Adding artificial "overrides" here for code-gen quality's sake.
  ...
  private emitAssemblyPackageInfo(mod: spec.Assembly) {
    ...
    if (mod.docs.deprecated) {
      this.code.line(' *');
      // Javac won't allow @deprecated on packages, while @Deprecated is absolutely fine. Duh.
  ...
  private emitMavenPom(assm: spec.Assembly, fingerprint: boolean) {
                    ...
                    },
                    {
                      ...
                      configuration: {
                        ...
                        sourceFileExcludes: {
                          ...
                          // are all collected at the top of the classlist, burying useful information under
    ...
    /**
     * Get the maven-style license block for the assembly.
    ...
    if (!prop.immutable) {
      for (const type of setterTypes) {
        ...
        if (prop.abstract && !defaultImpl) {
          ...
          // have no overlap (e.g. if a String instanceof Number).
          if (
  ...
  /**
   * Filters types from a union to select only those that correspond to the specified javaType.
   *
   ...
   * @return spec.TypeReference A type reference that matches the provided javaType.
   */
  private filterType(
    ...
    if (!spec.isUnionTypeReference(ref)) {
      // No filtering needed -- this isn't a type union!
      return ref;
    }
   ...
   *  3. Return the java FQN: ``<module.targets.java.package>.<FQN stripped of first component>``
   *
   * Records an assembly reference if the referenced FQN comes from a different assembly.
   *
   * @param  fqn         The JSII FQN to be used.
   * @param  binaryName  Whether to return the binary name (i.e. `$` instead of `.` as separator).
   * @return string The corresponding Java FQN.
   *
  ...
  private convertExample(example: string, api: rosetta.ApiLocation): string {
    const translated = this.rosetta.translateExample(
      api,
      example,
      rosetta.TargetLanguage.JAVA,
...
function translateFqn(
  ...
  while (fqn !== '' && fqn !== assm.name) {
    ...
    if (submodule) {
      ...
      implicitPackageNames.unshift(`.${codemaker.toSnakeCase(lastPart)}`);
    } else {
...
/**
 * Checks whether a name corresponds to a method on `java.lang.Object`. It is not
...
function myMarkDownToJavaDoc(source: string) {
  ...
  return rosetta.markDownToJavaDoc(source);
}
